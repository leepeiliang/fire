...
func NewFireBuffer(b []byte) *Buffer {
	var buf Buffer
	buf.WriteByteString(b)
	return &buf
}
...
func (b *Buffer) ReadString(read int) string {
	return string(b.ReadBytes(read))
}

func (b *Buffer) ReadBytes(read int) []byte {
	var n uint64
	if read == readUint8 {
		n = uint64(b.ReadInt8())
	}
	if read == readUint16 {
		n = uint64(b.ReadUint16())
	}
	if read == readUint32 {
		n = uint64(b.ReadUint32())
	}
