...
func (m *Variant) Decode(b []byte) (int, error) {
	...
	if n < 0 || n > MaxVariantArrayLength {
		...
		return buf.Pos(), fmt.Errorf("StatusBadEncodingLimitsExceeded")
	}
...
func (m *Variant) decodeValue(buf *Buffer) interface{} {
	...
	case TypeIDString:
		return buf.ReadString(readUint32)
	case TypeIDDateTime:
		return buf.ReadTime()
	//case TypeIDGUID:
	//	v := new(GUID)
	...
	case TypeIDByteString:
		return buf.ReadBytes(readUint32)
...
var errUnbalancedSlice = fmt.Errorf("unbalanced multi-dimensional array")
